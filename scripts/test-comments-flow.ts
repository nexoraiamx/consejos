...
async function testCommentsFlow() {
  ...
  try {
    ...
    const testUserIds = [userIdPostAuthor, userIdCommenter1, userIdCommenter2, userIdModerator];
    ...
    await db.delete(reputationEvents).where(inArray(reputationEvents.userId, testUserIds)).execute();
